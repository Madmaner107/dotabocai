	...
	var exportsMethods = {
		...
		newVerScrollForPull : function(wrapper,pulldownAction,pullupAction,opts,pullText){
			...
			scrollObj.on('scrollMove',function(){
				...
				if(this.options.pullUpToRefresh) {
					...
					if ($pullup.length>0 && this.y < this.minScrollY && this.y < (this.maxScrollY - 5) && !$pullup.hasClass('flip')) {
						$pullup.show();
						$pullup.removeClass().addClass('flip');
						$pullup.find('#pullup-label').html(releaseToLoading);
						this.maxScrollY = this.maxScrollY;
			...
			scrollObj.on('scrollStart', function(){
				...
				$("img.lazy").lazyload();
                defLiLength = $('[data-game-list]').length;
            });
